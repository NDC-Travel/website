...
import {countryNameToISO, getAddress, getCountryFromAddress, isoToFlag} from "@/components/my-carry";
...
export default function Listing() {
    const [packages, setPackages] = useState<Package[] | Carrier[] | any>([]);
    ...
    useEffect(() => {
        const fetchPackages = async () => {
            try {
                ...
                const req = await fetch('/api/carriers?limit=10');
                const items: Carrier[] = await req.json();
                ...
                const shuffled2 = items.sort(() => 0.5 - Math.random());

                console.log([...shuffled, ...shuffled2].sort(() => 0.5 - Math.random()))

                setPackages([...shuffled, ...shuffled2].sort(() => 0.5 - Math.random()))
            } catch (e) {
        ...
        <div key={`${pkg.id}-${index}`} className="!mx-0 md:!mx-3 !w-full md:!w-[400px]" style={{flexShrink: 0}}>
                ...
                pkg.type === 'package' ?
                    <Link href={`/package/${pkg.id}`} className="card h-100 hover-effect-scale">
                        <div className="card-img-top position-relative overflow-hidden">
                            <div
                                className="d-flex flex-row items-center !w-[90%] md:!w-[365px] justify-content-between gap-2 align-items-start position-absolute top-0 start-0 z-1 pt-1 pt-sm-0 ps-1 ps-sm-0 mt-2 mt-sm-3 ms-2 ms-sm-3">
                          ...
                          Colis pour des expedition
                            ...
                            <div className="ratio hover-effect-target bg-body-tertiary"
                                ...
                                <img className={'!h-[300px] !w-full !object-cover'}
                                     src={pkg.imageUrl || "https://img.freepik.com/free-vector/cardboard-box_23-2147513430.jpg?t=st=1760309231~exp=1760312831~hmac=cbb721d02a1fc426748066b66230a45e80ee69d96f18af4f1147f866ffb48b82&w=2000"}
                                     alt={pkg.packageContents}/>
                            </div>
                        </div>
                        ...
                        <div className="card-body pb-3">
                            <div className="d-flex align-items-center justify-content-between mb-4">
                                ...
                                <div className="mb-0 text-[0.85rem]">
                                    Indemnité Proposée: <b
                                    className="h6 text-primary">{pkg.participationAllowance} €</b>
                                </div>
                            </div>

                            <h3 className="h6 mb-2">
                                <Link href={`/package/${pkg.id}`}
                                      className="hover-effect-underline stretched-link me-1 !text-[#d46328]">
                                    {pkg.packageContents}
                                </Link>
                            </h3>
                                ...
                                <div className="d-flex align-items-center justify-content-between gap-2">
                                    ...
                                    <div className="d-flex align-items-center gap-2">
                                        <Avatar className={'!w-[32px] !bg-black !text-white !h-[32px]'}>
                                            ...
                                            <AvatarFallback
                                                className={'!text-decoration-none !bg-black !text-white'}>{pkg.user?.name?.charAt(0)}</AvatarFallback>
                                        </Avatar>
                                ...
                                </div>
                                {
                                    pkg.carrier !== null ? (
                                            <>
                                                <br/>
                                                <div
                                                    className="d-flex border-top align-items-center justify-content-between gap-2 pt-3">
                                                    Transporté par
                                                    <div className="d-flex align-items-center gap-2">
                                                        <Avatar className={'!w-[32px] !bg-black !text-white !h-[32px]'}>
                                                            <AvatarImage src={pkg.carrier?.image as string}/>
                                                            <AvatarFallback
                                                                className={'!text-decoration-none !bg-black !text-white'}>{pkg.carrier?.name?.charAt(0)}</AvatarFallback>
                                                        </Avatar>
                                        ...
                                        :
                                        <div
                                            className="d-flex border-top align-items-center justify-content-between gap-2 pt-3 mt-3">
                                            Pas de Transporteur
                                            <Link
                                            ...
                                            >
                                                <MessageSquareIcon className="w-4 me-2 h-4 text-white"/> Chat Par
                                                Message
                                            </Link>
                                        </div>
                                }
            ...
            <span className="badge text-bg-primary !bg-[#094786] fw-bold">
              Transporteur
                            ...
                            <div className="ratio hover-effect-target bg-body-tertiary"
                                 style={{'--fn-aspect-ratio': 'calc(204 / 306 * 100%)'} as React.CSSProperties}>
                                <img className={'!h-[300px] !w-full !object-cover'} src={"/carrier.jpeg"}
                                     alt={pkg.packageContents}/>
                            </div>
                        </div>
